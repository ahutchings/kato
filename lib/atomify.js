...
var regexes = {
  ...
  'sb': /^(.*\/)?(.+?)( \(20\d\d\))? S(\d+)E(\d\d\d?)(-\d\d)? (.+?)\....$/i, // Human Target (2010) S02E02 The Wife's Tale.mp4
  'other': /^(.*\/)?(.+?)( cd\d)?\....$/i
};
...
function lookupMetadata(filename, callback) {
  ...
  if (data[filename].tv_show) {
    if((match = data[filename].show.match(/^(NCIS|Criminal Minds|Green Lantern|Avatar|Tron|DreamWorks Dragons) (.*)/)) !== null){
      data[filename].show = data[filename].show_full = match[1] + ': ' + match[2]; // damn show names with colons!
    } else if((match = data[filename].show.match(/^Are You There, Chelsea/)) !== null) {
      data[filename].show = data[filename].show_full = 'Are You There, Chelsea?'; // every time you hardcode a show, a puppy dies. :-(
    }
