...
exports.atomify = function(file, callback) {
  lookupTVData(file, function(err, success) {
    if(err) throw err;
...
function lookupTVData(filename, callback) {
  //winston.debug("looking up data: " + filename);
  var arg, match, show, show_id, season, episode_padded, episode, name, title, airdate, description, longdesc;
  ...
  if ((match = arg.match(regexes.sb)) !== null) {
    ...
    show = match[2];
    season = parseInt(match[4], 10);
    episode_padded = match[5];
  ...
  if (show) {
    ...
    if((match = show.match(/^(NCIS|Criminal Minds) (.*)/)) !== null){
      show = match[1] + ': ' + match[2];
    }

    db.open('/Users/saadiq/dev/Sick-Beard/sickbeard.db', function(err) {
      if(err) throw err;
      db.query("select tvdb_id, show_name, genre from tv_shows where show_name like '"+show+"%'", [], function (err, row) {
        if(err) throw err;
        if (row === undefined) {
        ...
        data[filename].season = season;
        data[filename].episode = episode;
        data[filename].episode_id = formatEpId(season,episode);
