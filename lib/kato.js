  ...
  lookupMetadata(file, function(err, success) {
    ...
    if (success) {
      ...
      if(data[file].tv_show) {
        cmd = 'AtomicParsley \"'+outFile(file)+'\" --stik \"TV Show\" --TVSeason '+data[file].season+' --TVEpisode \"'+data[file].episode_id+'\" --TVEpisodeNum '+data[file].episode+' --TVShowName \"'+data[file].show+'\" --title \"'+data[file].title+'\" --disk '+data[file].season+' --tracknum '+data[file].episode+' --album \"'+data[file].show+'\" --artist \"'+data[file].show+'\" --genre \"TV Show\" --description \"'+data[file].description+'\" --longdesc \"'+data[file].description+'\" --overWrite';

        art = path.dirname(file) + '/folder.jpg';
      } else {
        //cmd = 'AtomicParsley \"'+outFile(file)+'\" --stik Movie --TVShowName \"'+data[file].show+'\" --title \"'+data[file].title+'\" --album \"'+data[file].show+'\" --artist \"'+data[file].show+'\" --year \"'+data[file].year+'\" --description \"'+data[file].description+'\" --longdesc \"'+data[file].description+'\" --overWrite';
        cmd = 'AtomicParsley \"'+outFile(file)+'\" --stik Movie --title \"'+data[file].name+'\" --year \"'+data[file].year+'\" --description \"'+data[file].overview+'\" --longdesc \"'+data[file].overview+'\" --overWrite';
...
function lookupMetadata(filename, callback) {
  ...
  if (data[filename].tv_show) {
    ...
    db.open(SB_PATH+'/sickbeard.db', function(err) {
      if(err) throw err;
      db.execute("select tvdb_id, show_name, genre from tv_shows where show_name like '"+data[filename].show+"%'", [], function (err, row) {
        if(err) throw err;

        //cliff.putObject(row);
        //data[filename].show = row.show_name;
        //data[filename].show = show;
        data[filename].show_id = row[0].tvdb_id;
        ...
        db.execute(sql, sql_args, function(err, rows) {
          if(err) throw err;

          cliff.putObject(rows);
          //data[filename].episode_id = row.episode_id;
          data[filename].episode_name = rows[0].name;
          data[filename].description = rows[0].description;
          //cliff.putObject(data);
    ...
    db.open(COUCH_PATH+'/data.db', function(err) {
      if(err) throw err;
      db.execute("select * from movie where name = ?", [data[filename].name], function (err, rows) {
        if(err) throw err;

        //cliff.putObject(row);
        data[filename].year = rows[0].year;
        data[filename].id = rows[0].id;

        db.execute("select * from movieextra where movieid = ?", [data[filename].id], function(err, rows) {
          if(err) throw err;
          
          for( var i = 0; i < rows.length; ++i) {
            data[filename][rows[i].name] = rows[i].value;
          }
          data[filename].art = COUCH_PATH + '/' + data[filename].poster_cover;
