...
function lookupMetadata(filename, callback) {
  ...
  if (data[filename].tv_show) {
    if((match = data[filename].show.match(/^(NCIS|Criminal Minds|Green Lantern) (.*)/)) !== null){
      data[filename].show = match[1] + ': ' + match[2]; // damn show names with colons!
    }

    db = new sqlite3.Database(SB_PATH+'/sickbeard.db', sqlite3.OPEN_READONLY, function(err) {
      if(err) throw err;
    ...
    db = new sqlite3.Database(COUCH_PATH+'/data.db', sqlite3.OPEN_READONLY, function(err) {
